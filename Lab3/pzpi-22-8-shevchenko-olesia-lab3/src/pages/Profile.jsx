            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <div className="mt-4 text-sm text-white/50">
                  <span className="mr-6">
                    <strong className="text-white">{profile.observation_count || 0}</strong> {t("profile.observationsCount")}
                  </span>
                  <span>
        ...
        {observations.map((obs) => (
          ...
          >
            ...
            <div className="flex space-x-4">
              <button
                onClick={() => {
                  if (obs?.id) navigate(`/observations/${obs.id}/edit`);
                }}
            ...
            <div className="flex flex-col text-left w-full">
              <div>
                ...
                <div className="text-sm text-white/60">
                  ...
                  >
                    {showObjects
                      ? t("profile.hideObjects")
                      : `${t("profile.showObjects")} (${selectedObservation.objects_in_field?.length || 0})`}
                  </h4>
                  {showObjects && (
                ...
                {selectedObservation.location?.place_name && (
                  <div className="text-base italic text-white/75 mt-5">
                    <strong>{t("profile.location")}:</strong> {selectedObservation.location.place_name}
                  </div>
                )}
              </div>
              ...
              <div className="mt-6">
                <button
                  onClick={closeModal}
                  className="w-full mt-5 px-6 py-1 border-white text-white font-serif rounded-lg hover:bg-yellow-600 transition"
                >
                  {t("profile.close")}
                </button>
              </div>
