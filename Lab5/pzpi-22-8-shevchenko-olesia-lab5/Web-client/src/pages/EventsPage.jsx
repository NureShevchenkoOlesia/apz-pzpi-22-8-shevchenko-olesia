...
export default function Events() {
  ...
  return (
    <div className="events-container">
      <h2 className="events-title">Upcoming Solar Events</h2>
        ...
        {filteredEvents.map((event) => (
          ...
          >
            ...
            <p className="text-sm text-white/50 line-clamp-3">
              {event.type === "FLR" ? (
                <span>Class: {event.class_type}</span>
              ) : (
                event.kp_index?.map((entry, idx) => (
                  <span key={idx}>
                    {new Date(entry.observedTime).toLocaleDateString()}: KpIndex{" "}
                    {entry.kp_index}{" "}
                  </span>
                ))
              )}
            ...
            >
              {followingEvents[event._id] ? "Following" : "Follow"}
            </button>
          </div>
        ))}
      </div>
          ...
          >
            ...
            <p className="text-white/70 mb-4">
              {selectedEvent.type === "FLR" ? (
                <span>Class: {selectedEvent.class_type}</span>
              ) : (
                selectedEvent.kp_index?.map((entry, idx) => (
                  <div key={idx}>
                    <p>
                      {new Date(entry.observedTime).toLocaleString()} - KpIndex:{" "}
                      {entry.kp_index}
                    </p>
                  </div>
                ))
